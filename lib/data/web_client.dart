...
class WebClient {
  ...
  void _checkResponse(String url, http.Response response) {
    if (!kReleaseMode) {
      logInfo('Response: ${formatSize(response.bodyBytes.length)}');
      if (Config.DEBUG_REQUESTS) {
        printWrapped('${response.statusCode} ${response.body}');
        logInfo('Headers: ${response.headers}');
      }
    }
    ...
    try {
      ...
      if (jsonResponse['errors'] != null &&
        ...
        try {
          ...
          logError('Failed to parse error: $error');
        }
      }
    } catch (error) {
      logError('Failed to parse error: $error');
