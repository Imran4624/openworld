...
class DynamicFieldsCreate extends StatefulWidget {
  const DynamicFieldsCreate({super.key});

  static const String route = '/DynamicFieldsCreate';

  @override
  _DynamicFieldsCreateState createState() => _DynamicFieldsCreateState();
}

class _DynamicFieldsCreateState extends State<DynamicFieldsCreate> {
  late Store<AppState> _store;
  int _currentSliderIndex = 0;
  int _totalSliderCount = 0;
  ...
  Color _getNextButtonTextColor(
    bool canMoveNext,
    bool isShowingSlider,
    bool isLastSlide,
    int currentQuestionIndex,
    QuestionGroupModel currentGroup,
    Color enabledColor,
    Color disabledColor,
  ) {
    if (isShowingSlider) {
      return isLastSlide ? enabledColor : disabledColor;
    }
    if (currentQuestionIndex == -1 && currentGroup.showIntroScreen) {
      return enabledColor;
    }
    return canMoveNext ? enabledColor : disabledColor;
  }
  ...
  VoidCallback? _getNextButtonAction(
    ...
    if (vm.currentQuestionIndex == -1 &&
      ...
      return () => vm.moveNext(context);
    }
    return vm.canMoveNext
        ? (isLastQuestion
            ? handleSubmit
            : () {
                vm.moveNext(context);
        ...
        if (vm.currentQuestionIndex == -1) {
          if (currentGroup.showSlider && !_hasViewedSlider) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                setState(() {
                  _totalSliderCount = sliderImages.length;
                });
              }
            });
  ...
  Widget _buildQuestionView(_ViewModel vm, QuestionGroupModel currentGroup,
    ...
    if (currentGroup.showAllQuestions) {
      ...
      print('vm controllers ==> ${vm.inputFieldControllers}');
    ...
    return StoreConnector<AppState, _ViewModel>(
      ...
      builder: (context, vm) {
        ...
        List<QuestionModel> visibleQuestions = editQuestions.where((question) {
          ...
          final builtAnswers = answersMap != null
              ? BuiltMap<String, BuiltMap<String, dynamic>>(answersMap)
              : BuiltMap<String, BuiltMap<String, dynamic>>();
        ...
        final isShowingSlider = vm.currentQuestionIndex == -1 &&
            ...
            !_hasViewedSlider;
        final isLastSlide = _currentSliderIndex == _totalSliderCount - 1;

        final textStyle = Theme.of(context)
            .textTheme
            .bodyMedium!
            .copyWith(color: store.state.headerTextColor);
        final name = vm.questionGroupType.getName();
        final isLastQuestion = vm.currentGroupIndex ==
                vm.questionGroups.length - 1 &&
            (vm.currentQuestionIndex == visibleQuestions.length - 1 ||
                vm.questionGroups[vm.currentGroupIndex].showAllQuestions &&
                    !vm.questionGroups[vm.currentGroupIndex].showIntroScreen &&
        ...
        return Scaffold(
          appBar: isProfileCreate || profileBeingEdited
              ? null
              : AppBar(
                  title: Text(name),
                  ...
                  children: [
                    if (!isShowingSlider &&
                      ...
                      OutlinedButton(
                        ...
                        onPressed: vm.movePrevious,
                        child: Text('Previous',
                            style: textStyle.copyWith(
                                color: AppTheme.light.secondary)),
                      ),
                    const Spacer(),
                    if (!isShowingSlider || isLastSlide)
                      OutlinedButton(
                        style: ButtonStyle(
                            backgroundColor: WidgetStateProperty.all(
                                AppTheme.light.primary)),
