  ...
  Widget build(BuildContext context) {
    ...
    if (ProjectConfig.fullWidthEntities().contains(EntityType.event) &&
      ...
      if (widget.isTopFilter) {
        return Material(
          ...
          child: Column(
            ...
            children: [
              if (ProjectConfig.showEntityTopBar(
                  EventScreen.route, store.state.prefState.isMobile))
                const EntityTopFilterHeader(),
              Expanded(
                child: ProjectConfig.appType == AppType.loopjam
                    ? EventViewMainBanner(
                        key: ValueKey(viewModel.event.id), viewModel: viewModel)
                    : EventViewFullwidth(viewModel: viewModel),
              ),
            ],
          ),
        );
      ...
      return ViewScaffold(
        ...
        isEditable: !isGuestUser(store.state),
        entity: event,
            ...
            children: [
              Column(
                children: [
                  ...
                  if (ProjectConfig.showDetailBottomButtonsForFullScreen() &&
                      !isGuestUser(store.state) &&
                      (isAdmin(store.state) ||
                          event.createdUserId == getLoggedInUserId(store)))
                    BottomButtons(
                      entity: event,
                      action1: EntityAction.delete,
                      action2: EntityAction.archive,
                    ),
                  const SizedBox(height: 72.0),
                ],
              ),
              if (!event.id.contains('_isPreview'))
                FloatingActionButtons(
                  entityType: EntityType.event,
                  backgroundColor: event.themeObject?.accentColor ?? AppTheme.light.primary,
                  showEditButton:
                      event.createdUserId == getLoggedInUserId(store),
                  showLinkButton: isAuthenticated(store.state),
                  showBackButton: true,
                  showAttendeesButton:
                      ProjectConfig.appType == AppType.loopjam &&
                          event.createdUserId == getLoggedInUserId(store),
                        ...
                        try {
                          final result = await UserService.updateUserPaymentStatus(
                            ...
                            metadata: {
                              ...
                              'upgrade_source': 'event_view_photo_upload',
                          ...
                          if (result['success'] == true) {
                            debugPrint('User upgraded to Premium plan for photo uploads - profile updated');
                          } else {
                            debugPrint('User upgraded to Premium plan for photo uploads - profile update failed: ${result['error']}');
                          }
                        } catch (e) {
                  ...
                  onEdit: () {
                    store.dispatch(EditEvent(event: event));
                  },
                  onLink: () {
                        ...
                        try {
                          final result = await UserService.updateUserPaymentStatus(
                            ...
                            metadata: {
                              ...
                              'upgrade_source': 'event_view_share',
                              'upgrade_date': DateTime.now().toIso8601String(),
                              'event_id': event.id,
                            },
                          );
                          ...
                          if (result['success'] == true) {
                            debugPrint('User upgraded to Premium plan - profile updated');
                          } else {
                            debugPrint('User upgraded to Premium plan - profile update failed: ${result['error']}');
                          }
                        } catch (e) {
                          debugPrint('Error updating user profile after payment: $e');
                        }
