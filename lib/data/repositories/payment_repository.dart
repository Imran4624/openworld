...
class PaymentRepository {
  ...
  Future<PaymentEntity> loadItem(
    ...
    if (data != null) {
      return serializers.deserializeWith(PaymentEntity.serializer, data)!;
    }
    throw Exception('Payment with ID $entityId not found');
  }
  ...
  Future<BuiltList<PaymentEntity>> loadList(Credentials credentials) async {
    ...
    final payments = dataList
        .map((data) =>
            serializers.deserializeWith(PaymentEntity.serializer, data)!)
        .toList();

    return BuiltList<PaymentEntity>(payments);
  }
  ...
  Future<Map<String, dynamic>> loadListWithPagination({
    ...
    return {
      'payments': BuiltList<PaymentEntity>(payments),
  ...
  Future<List<PaymentEntity>> bulkAction(
    ...
    return ids.map((id) => PaymentEntity(id: id)).toList();
  }
  ...
  Future<PaymentEntity> saveData(
    ...
    if (payment.id.isEmpty ||
        !(await _firebaseRepository.itemExists(payment.id))) {
      final newId = _firestore.collection('payments').doc().id;
      data['id'] = newId;

      await _firebaseRepository.saveItem(newId, data);
      return payment.rebuild((b) => b..id = newId);
    } else {
      await _firebaseRepository.saveItem(payment.id, data);
      return payment;
    }
  }
}
