...
class WorkoutRepository {
  ...
  static final FirebaseRepository _firebaseRepository =
      FirebaseRepository('workouts');
      
  static final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  Future<WorkoutEntity> loadItem(
      Credentials credentials, String entityId) async {
    final data = await _firebaseRepository.getItem(entityId);
    if (data != null) {
      return serializers.deserializeWith(WorkoutEntity.serializer, data)!;
    }
    throw Exception('Workout with ID $entityId not found');
  }

  Future<BuiltList<WorkoutEntity>> loadList(Credentials credentials) async {
    final response = await _firebaseRepository.getList();
    final dataList = response['data'] as List<Map<String, dynamic>>;
    final workouts = dataList
        .map((data) =>
            serializers.deserializeWith(WorkoutEntity.serializer, data)!)
        .toList();

    return BuiltList<WorkoutEntity>(workouts);
  }
  ...
  final workouts = dataList
     .map((data) => serializers.deserializeWith(WorkoutEntity.serializer, data)!)
      .toList();

  return {
    'workouts': BuiltList<WorkoutEntity>(workouts),
    'lastDocument': lastDocumentSnapshot,
  };
}

  Future<List<WorkoutEntity>> bulkAction(
    ...
    return ids.map((id) => WorkoutEntity(id: id)).toList();
  }

  Future<WorkoutEntity> saveData(
      Credentials credentials, WorkoutEntity workout) async {
    final data = serializers.serializeWith(WorkoutEntity.serializer, workout)
        as Map<String, dynamic>;

    if (workout.id.isEmpty ||
        !(await _firebaseRepository.itemExists(workout.id))) {
      final newId = _firestore.collection('workouts').doc().id;
      data['id'] = newId;

      await _firebaseRepository.saveItem(newId, data);
      return workout.rebuild((b) => b..id = newId);
    } else {
      await _firebaseRepository.saveItem(workout.id, data);
      return workout;
    }
  }
}
