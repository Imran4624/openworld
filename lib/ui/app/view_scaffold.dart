...
class ViewScaffold extends StatelessWidget {
        ...
        builder: (BuildContext context) {
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                    'Please describe why you are reporting this ${entity.entityType?.name}:'),
    ...
    return FocusTraversalGroup(
      child: Scaffold(
        ...
        appBar: !ProjectConfig.showEntityTopBar(
                state.uiState.currentRoute, isMobile(context))
            ? null
            : AppBar(
                centerTitle: false,
                leading: leading,
                ...
                actions: entity.isNew ||
                        !ProjectConfig.showActionButtonByEntityType(
                            entity.entityType!)
                    ? []
                    : [
                                  ...
                                  showDialog(
                                    ...
                                    builder: (BuildContext dialogContext) {
                                      ...
                                      final formKey = GlobalKey<FormState>(
                                          debugLabel: '_reportProfileDialog');
                        ...
                        ((!ProjectConfig.showActionButtonByEntityType(
                                    entity.entityType!)) ||
                                entityActions(userCompany, store).isEmpty)
                            ? SizedBox()
                            : ViewActionMenuButton(
                                ...
                                onSelected: (context, action) =>
                                    handleEntityAction(entity, action,
                                        autoPop: true),
                                entityActions:
                                    entityActions(userCompany, store),
                              ),
                      ],
              ),
        body: SafeArea(
  ...
  List<EntityAction?> entityActions(
      UserCompanyEntity userCompany, Store<AppState> store) {
    final isAuthor = entity.createdUserId == getLoggedInUserId(store);

    return isAuthor || isAdmin(store.state)
        ? entity.getActions(
